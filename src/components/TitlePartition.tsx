...
const Partition: React.FC<PartitionProps> = ({
  ...
  // Mesurer la largeur du texte à chaque changement
  useEffect(() => {
  ...
  useEffect(() => {
    const updateSize = () => {
      const clefWidth = showClef ? lineGap * 12 : 0;
      const spacing = lineGap * 4; // espace avant et après le texte
      const totalWidth = clefWidth + spacing * 2 + textWidth;

      const height = paddingY * 2 + (numberOfLines - 1) * lineGap;
  ...
  const clefSize = lineGap * 12;
  const clefY = paddingY - clefSize * 0.12;
  const spacing = lineGap * 4;

  // Position du texte centré dans l’espace entre les lignes
  const textX = (showClef ? clefSize : 0) + spacing + textWidth / 2;
  const textY = paddingY + ((numberOfLines - 1) * lineGap) / 2;

  // Position et longueur des lignes
  const lineStart = (showClef ? clefSize : 0);
  const lineEnd = lineStart + spacing * 2 + textWidth;

  return (
    <div ref={containerRef} style={{justifyContent: "center", display: "flex", width: "100%"}}>
      <svg
